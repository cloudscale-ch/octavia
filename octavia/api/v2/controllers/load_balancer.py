...
class LoadBalancersController(base.BaseController):
    ...
    @staticmethod
    def _validate_subnets_share_network(load_balancer, subnet_id=None):
        # Validate that all subnets belong to the same network
        network_driver = utils.get_network_driver()
        if not subnet_id and hasattr(load_balancer, 'vip_subnet_id'):
            subnet_id = load_balancer.vip_subnet_id
        else:
            # Without a known subnet for the VIP, validation is not possible at
            # this stage
            # TODO(gaudenz): How to improve validation for PUT requests
            return
        subnet_network_map = {
            subnet_id:
    ...
    def _validate_vip_request_object(self, load_balancer, context=None):
        ...
        self._validate_subnets_share_network(load_balancer)
    ...
    def put(self, id, load_balancer):
        ...
        if not isinstance(load_balancer.additional_vips, wtypes.UnsetType):
            self._validate_subnets_share_network(
                load_balancer,
        ...
        with db_api.get_lock_session() as lock_session:
            ...
            if 'additional_vips' in db_lb_dict:
                ...
                for vip in (new_additional_vips.keys() -
                    ...
                    self.repositories.additional_vip.create(
                        lock_session,
                        load_balancer_id=id,
                        **new_additional_vips[vip],
                    )
