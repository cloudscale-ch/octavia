...
class TestHaproxyAmphoraLoadBalancerDriverTest(base.TestCase):
    ...
    def setUp(self):
        ...
        self.sl = sample_configs_split.sample_listener_tuple(
            ...
            recursive_nest=True)
        self.sl_udp = sample_configs_split.sample_listener_tuple(
        ...
        self.pool_has_cert = sample_configs_split.sample_pool_tuple(
            pool_cert=True, pool_ca_cert=True, pool_crl=True)
        self.amp = self.sl.load_balancer.amphorae[0]
        self.sv = sample_configs_split.sample_vip_tuple()
        self.lb = self.sl.load_balancer
        self.lb_udp = (
            sample_configs_split.sample_lb_with_udp_listener_tuple())
    ...
    def test_update_amphora_listeners(self, mock_load_cert, mock_secret):
        ...
        mock_load_cert.return_value = {
            ...
            'client_ca_cert': None}
        self.driver.jinja_split.build_config.return_value = 'the_config'
        ...
        self.driver.clients[API_VERSION].upload_config.assert_called_once_with(
            mock_amphora, self.sl.id, 'the_config',
            timeout_dict=self.timeout_dict)
        self.driver.clients[API_VERSION].reload_listener(
            mock_amphora, self.sl.id, timeout_dict=self.timeout_dict)

        mock_load_cert.reset_mock()
        self.driver.jinja_split.build_config.reset_mock()
    ...
    def test_update(self, mock_cert, mock_load_crt, mock_secret):
        ...
        self.driver.clients[API_VERSION].get_cert_md5sum.side_effect = [
            ...
            'CA_CERT_MD5']
        self.driver.jinja_split.build_config.side_effect = ['fake_config']
        ...
        self.driver.clients[API_VERSION].upload_config.assert_called_once_with(
            self.amp, self.sl.id, 'fake_config', timeout_dict=None)
        # start should be called once
        self.driver.clients[
            API_VERSION].reload_listener.assert_called_once_with(
            self.amp, self.sl.id, timeout_dict=None)
        secret_calls = [
    ...
    def test_delete_second_listener(self, mock_cert, mock_load_crt,
        ...
        sl = sample_configs_split.sample_listener_tuple(
            tls=True, sni=True, client_ca_cert=True, client_crl_cert=True,
            recursive_nest=True)
        sl2 = sample_configs_split.sample_listener_tuple(
        ...
        # Now just make sure we did a delete
        self.driver.clients[
...
class TestAmphoraAPIClientTest(base.TestCase):
    ...
    @requests_mock.mock()
    def test_get_api_version_not_found(self, mock_requests):
        mock_requests.get('{base}/'.format(base=self.base_url),
                          status_code=404)
        self.assertRaises(exc.NotFound, self.driver.get_api_version, self.amp)
